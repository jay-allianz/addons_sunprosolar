...
class account_balance(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            'time': time,
            'lines': self.lines,
            'comp_1_line':self.comparison1_lines,
            ...
            'second_fiscalyear':self.get_second_fiscalyear,
            'third_fiscalyear':self.get_third_fiscalyear,
            'get_month_periodic':self.get_month_periodic
        })
        self.context = context

    def get_second_fiscalyear(self, form):
        """
        ...
        if form.get('compr0_fiscalyear_id'):
            fiscalyear = fiscalyear_obj.browse(self.cr, self.uid, form['compr0_fiscalyear_id'][0])
    ...
    def get_third_fiscalyear(self, form):
        """
        Returns the fiscal year text used on the report.
        """
        fiscalyear_obj = self.pool.get('account.fiscalyear')
        ...
        if form.get('compr1_fiscalyear_id'):
            fiscalyear = fiscalyear_obj.browse(self.cr, self.uid, form['compr1_fiscalyear_id'][0])
            return fiscalyear.name or fiscalyear.code
        return ''
    ...
    def get_month_periodic(self, form):
        ...
        if not form['filter']:
            ...
            date = 'As Of '+ to_date.strftime("%B %d, %Y")
            return date
        elif form['inf_type'] in ['BS'] and form['filter'] in ['byperiod', 'all']:
            aux = []
            for period in period_obj.browse(self.cr, self.uid, form['periods']):
                aux.append(period.date_stop)
            sorted(aux)
            ...
            return date
        elif form['inf_type'] in ['IS'] and form['filter'] in ['byperiod', 'all']:
            aux=[]
            for period in period_obj.browse(self.cr, self.uid, form['periods']):
                aux.append(period.date_start)
                aux.append(period.date_stop)
            ...
            date = 'As Of '+ to_date.strftime("%B %d, %Y")
            return date
        elif form['inf_type'] in ['TB'] and form['filter'] in ['byperiod', 'all']:
            aux = []
            for period in period_obj.browse(self.cr, self.uid, form['periods']):
                aux.append(period.date_stop)
            sorted(aux)
...
report_sxw.report_sxw('report.periodic.2cols', 
                      'wizard.report', 
                      'addons/sunpro_account_financial_report/report/periodic_financial_report_1_2cols.rml',
                       parser=account_balance, 
                       header=False)

report_sxw.report_sxw('report.periodic.4cols', 
                      'wizard.report', 
