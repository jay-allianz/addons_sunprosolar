...
class third_party_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        self.partner_ids = data['form']['partner_ids']
        
        ctx = data['form'].get('used_context')
        ctx.update({'partner_ids' : self.partner_ids})
        ...
        if (data['model'] == 'res.partner') and ids:
            ...
            params += [tuple(ids)]
        if self.partner_ids:
            PARTNER_REQUEST1 =  "AND l.partner_id IN %s"
            params += [tuple(self.partner_ids)]
        self.cr.execute(
...
report_sxw.report_sxw('report.account.third_party_ledger1', 'res.partner',
        'addons/account/report/account_partner_ledger.rml',parser=third_party_ledger,
        header='internal')

report_sxw.report_sxw('report.account.third_party_ledger_other1', 'res.partner',
